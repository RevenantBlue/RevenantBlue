		...
		try {
			...
			$stmt = self::$dbh->prepare(
				"UPDATE $this->linkCatTable
				 SET category_title = :categoryTitle, category_alias = :categoryAlias, category_description = :categoryDescription, category_image = :categoryImage
				 WHERE id = :id"
			);
			$stmt->bindParam(':id', $linkCat['id'], PDO::PARAM_INT);
			$stmt->bindParam(':categoryTitle', $linkCat['name'], PDO::PARAM_STR);
			$stmt->bindParam(':categoryAlias', $linkCat['alias'], PDO::PARAM_STR);
