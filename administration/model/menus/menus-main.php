...
class Menus extends Db {
	...
	public function insertMenu($menu, $parentId = NULL) {
		try {
			...
			$stmt->bindParam(':published', $menu['published'], PDO::PARAM_INT);
	...
	public function updateMenu($menu) {
		try {
			...
			self::$dbh->rollBack();
			if(!empty($menu['image_path']) && !empty($menu['image']) && is_file(DIR_IMAGE . 'menus/' . $menu['image'])) {
				unlink(DIR_IMAGE . 'menus/' . $menu['image']);
			}
			$this->errorLog($e);
		}
	}
