...
class LoginValidation extends GlobalValidation {
	...
	public function __construct($login, $password, $loginType = 'username') {
		...
		if($loginType === 'username') {
			$this->loginValidation($login, $password);
		} elseif($loginType === 'email') {
			$this->emailLoginValidation($login, $password);
		}
	}
		...
		if(!$user['activated']) {
			...
			if($globalSettings['new_user_email_verify']['value'] == 1) {
				...
				if(!empty($emailVerified)) {
					...
					header('Location: ' . $_SERVER['REQUSET_URI'], TRUE, 302);
					exit;
				}
			}
		}
...
class UserValidation extends GlobalValidation {
	...
	public function validateName($name, $nameType) {
		...
		if(empty($name) && $this->strict === TRUE) {
			$this->errors[] = "No $nameType provided";
		}
		return trim($name);
	}
	public function validateUsername($username) {
		...
		if(preg_match('/[^a-zA-Z0-9_ ]/', $username)) {
			$this->errors[] = "The username can only contain letters, numbers, spaces, and underscores.";
		}
	...
	public function validatePassword($password) {
		...
		if($this->strict === TRUE) {
			...
			if(!preg_match('/[A-Z]/', $password)) {
				$this->errors[] = "Your password must contain at least one capital letter.";
			}
			// Test the password for a number.
			if(!preg_match('/[0-9]/', $password)) {
				$this->errors[] = "Your password must contain at least one number.";
			}
		}
	...
	public function sendUserEmail($subject, $body, $toEmail, $toFirst, $toLast, $fromEmail, $fromName, $toId) {
		...
		shell_exec('php ' . DIR_SYSTEM . 'cron-jobs/send-mail.php &');
		...
		if(empty($this->errors)) {
			...
			if(!empty($this->id) && !empty($createUserRoles)) {
				...
				if($this->globalSettings['new_user_email_verify']['value']) {
					...
					// Manually run the send mail script to ensure the user gets their email verification promptly.
					shell_exec('php5 ' . DIR_SYSTEM . 'cron-jobs/send-mail.php');
				}
				return true;
			} else {
