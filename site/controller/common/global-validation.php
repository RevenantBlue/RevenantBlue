...
class GlobalValidation {
	...
	public function validateAlias($alias, $checkForDuplicate = TRUE) {
		...
		if($checkForDuplicate === TRUE) {
			$this->alias = $alias;
			$num = 2;
			while($this->checkForDuplicateAlias($this->alias)) {
	...
	public function validateCreateDate($createDate = NULL) {
		...
		if(empty($createDate)) {
			...
			if($validDate === TRUE) {
				$this->createDate = date('Y-m-d H:i:s', $date);
				return $this->createDate;
			} else {
				$this->createDate = $createDate;
				$this->errors[] = "The created date entered is invalid.  The correct date/time format is MM-DD-YY HH:MM AM/PM";
			}
		}
	}
	...
	public function validateContent($content, $contentFormatId, $allowEmpty = FALSE, $emptyError = FALSE) {
		...
		$this->content = ContentFilter::filterContent($content, $contentFormatId);
		return $this->content;
	}
